...
impl Entities {
    ...
    /// allocations in [`Archetype`]s and [`Table`]s.
    pub fn alloc(
        &mut self,
        ...
        if let Some(EntityEntry { entry, generation }) = self.entities.get_mut(self.free_head) {
            if let Entry::Free { next_free } = entry {
                ...
                match f(entity) {
                    ...
                    // The usage of the newly created entity failed, so we ignore it and do nothing
                    // about it here. Ideally the implementor of `f` should handle the error.
                    Err(_) => (),
                }
            } else {
